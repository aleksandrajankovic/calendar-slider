import promotions from "./promotions.js";
    ...
    document.addEventListener("DOMContentLoaded", function() {
      ...
      function updateSlider() {
        ...
        const computedGap = parseFloat(getComputedStyle(document.querySelector(".slider-track")).gap) || 0;
        const cellWidth = (sliderContainer.clientWidth - (slidesToShow - 1) * computedGap) / slidesToShow;
        track.style.transform = `translateX(-${currentIndex * (cellWidth + computedGap)}px)`;
        ...
        if (slidesToShow === 3) {
          ...
          if (info) {
            ...
            info.innerHTML = `<div class="slide-info"><span class="date">${dayText}</span>${slides[firstIndex].classList.contains("no-click") ? "" : `<br><span class="title">${titleText}</span>`}</div>`;
          }
          slides.forEach(slide => slide.classList.remove("center","left","right"));
        }
      }
